...
void displayBSTdecorated(BST*t, FILE *fp){
	...
	while(1){
		...
		while (nodeCount > 0){
			...
			if(getBSTNODEleft(search) == NULL && getBSTNODEright(search) == NULL)
				fprintf(fp,"=");
			t->display(getBSTNODEvalue(search),fp);
			if(level == 0){
				fprintf(fp,"(");
				t->display(getBSTNODEvalue(getBSTroot(t)),fp);
				fprintf(fp,")");
				fprintf(fp,"X");
			}
			else{
				if(getBSTNODEleft(getBSTNODEparent(search)) == search){
				
				fprintf(fp,"(");
				t->display(getBSTNODEvalue(getBSTNODEparent(search)),fp);
				fprintf(fp,")");
				fprintf(fp,"L");
				}
				else if(getBSTNODEright(getBSTNODEparent(search)) == search){
